...
pub fn layout_flowchart(
    ...
    let mut positioned_subgraphs =
        compound::position_subgraphs(&ast.subgraphs, &positioned_nodes, &ast.style_overrides, measurer, &membership);
    ...
    // 10. Extract bend points and label dimensions from dummy chains, then route edges
...
/// Result of extracting bend points and label dimensions from dummy chains.
...
/// and extract label dimensions from label dummy metadata.
///
...
fn build_edge_bend_points(
    ...
    for chain in dummy_chains {
        ...
        if let Some(label_dummy) = chain.label_node {
            if positions.get(&label_dummy).is_some() {
