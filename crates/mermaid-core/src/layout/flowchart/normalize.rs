...
mod tests {
    ...
    fn test_min_from_subgraphs_only() {
        ...
        let mut nodes = vec![];
        let mut edges = vec![];
        let mut subgraphs = vec![make_sg("SG1", 10.0, 20.0), make_sg("SG2", -5.0, -10.0)];
    ...
    fn test_negative_coordinates_shifted() {
        ...
        assert!(
            nodes[0].x >= 0.0,
            "node x should be non-negative after normalize"
        );
        assert!(
            nodes[0].y >= 0.0,
            "node y should be non-negative after normalize"
        );
    }

    #[test]
    fn test_edges_shifted_with_nodes() {
        let mut nodes = vec![make_node("A", -50.0, -30.0), make_node("B", 50.0, 70.0)];
        ...
        for point in &edges[0].points {
            assert!(
                ...
                "edge point should be non-negative"
            );
        }
    ...
    fn test_edge_without_label() {
        let mut nodes = vec![make_node("A", 0.0, 0.0), make_node("B", 100.0, 100.0)];
        let mut edges = vec![PositionedEdge {
            from_id: "A".into(),
            to_id: "B".into(),
            line_style: LineStyle::Solid,
    ...
    fn test_edge_label_x_only_shifted() {
        ...
        assert!(
            edges[0].label_x.unwrap() >= 0.0,
            "label_x should be shifted"
        );
        assert!(edges[0].label_y.is_none());
    }
    ...
    fn test_edge_label_y_only_shifted() {
        ...
        assert!(edges[0].label_x.is_none());
        assert!(
            edges[0].label_y.unwrap() >= 0.0,
            "label_y should be shifted"
        );
    }
    ...
    fn test_subgraphs_negative_coords_shift() {
        ...
        let mut nodes = vec![];
        let mut edges = vec![];
        let mut subgraphs = vec![make_sg("SG1", -100.0, -50.0), make_sg("SG2", 10.0, 10.0)];

        normalize_and_compute_bounds(&mut nodes, &mut edges, &mut subgraphs);

        assert!(
            subgraphs[0].x >= -0.1,
            "subgraph with negative x should be shifted"
        );
        assert!(
            subgraphs[0].y >= -0.1,
            "subgraph with negative y should be shifted"
        );
        assert!(
            (subgraphs[1].x - 110.0).abs() < 0.1,
            "second subgraph shifted by 100"
        );
        assert!(
            (subgraphs[1].y - 60.0).abs() < 0.1,
            "second subgraph shifted by 50"
        );
    }
}
