...
mod tests {
    ...
    cyr[Cyrillic]-->cyr2((Circle shape Начало));
     ...
     class di orange"#;
        ...
        for node in &ast.nodes {
            println!("  Node: {} shape={:?} label={:?}", node.id, node.shape, node.label.as_deref().unwrap_or(""));
        }
        
        println!("\nSubgraphs: {}", ast.subgraphs.len());
        for sg in &ast.subgraphs {
            println!("  Subgraph: {} nodes={} edges={}", sg.id, sg.nodes.len(), sg.edges.len());
        ...
        for edge in &ast.edges {
            println!("  Edge: {} -> {} (type={:?} label={:?})", edge.from, edge.to, edge.edge_type, edge.label);
        }
    }
    ...
    subgraph A
        ...
        for sg in &ast.subgraphs {
            ...
            for node in &sg.nodes {
                println!("    Node: {} shape={:?} label={:?}", node.id, node.shape, node.label);
            }
        ...
        assert_eq!(od_node.label.as_deref(), Some("Odd shape"), "od should have label 'Odd shape'");
    ...
    class sq green"#;
        ...
        println!("\n=== Style Resolution ===");
        for (id, (node, style)) in &all_nodes {
            println!("Node: {}", id);
    ...
    class sq green"#;
        ...
        let svg = render(source, &config).unwrap();
    ...
    cyr[Cyrillic]-->cyr2((Circle shape Начало));
     ...
     class di orange"#;
        ...
        for node in &["sq", "ci", "od", "ro", "di", "ro2", "od3", "e", "f", "cyr", "cyr2"] {
    ...
    #[test]
    fn test_case5_full_render_debug() {
        use crate::{render, RenderConfig};
